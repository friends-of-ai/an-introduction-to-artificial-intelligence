...
Auf dem Gebiet der künstlichen Intelligenz wird schon seit einigen Jahrzehnten geforscht. Die Algorithmen und Vorgehensweisen unterscheiden sich dabei heute gar nicht so grundlegend von den Vorgehensweisen vor rund 20 Jahren. Was den Hype und den Erfolg dieser Technik heutzutage ausmacht sind die Rechenleistung ganzer Rechenzentren, der Zugang zu kostengünstigen "menschlichen" Ressourcen (durch die Nutzen von Internetdiensten wie Google, Microsoft, facebook, etc.), sowie eine schier unendlich verfügbare Menge an schon vorhandenen Daten (klassifiziert und noch zu klassifizieren).
